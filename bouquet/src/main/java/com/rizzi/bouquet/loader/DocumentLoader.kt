...
class RealDocumentLoader(
    ...
    override suspend fun execute(request: DocumentRequest): DocumentResult = withContext(Dispatchers.IO) {
        val eventListener = (eventListenerFactory?.value ?: EventListener.Factory.Empty).create(request)
                        ...
                        outFile.outputStream().use { output ->
                            ...
                            while (count != -1) {
                                ...
                                eventListener.onLoading(
                                    request,
                                    progress * (100 / totalBytes.toFloat())
                                )
                            }
                        }
                    }
                    ...
                    if (!hasCache) {
                        val response = (callFactoryLazy?.value ?: DefaultCallFactory).newCall(
                            Request.Builder()
                                ...
                                .build()
                        ).execute().body ?: throw IllegalStateException("Response body cannot be empty...")
                            ...
                            file.outputStream().use { output ->
                                ...
                                while (count != -1) {
                                    if (totalBytes > 0) {
                                        ...
                                        eventListener.onLoading(
                                            request,
                                            downloaded * (100 / totalBytes.toFloat())
                                        )
                                    }
                                    output.write(data, 0, count)
                                    data = ByteArray(bufferSize)
